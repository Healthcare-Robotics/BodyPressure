...
if __name__ == '__main__':
    ...
    #for gpsn in [["f", "rside", 1922, 7677, "21to30", "train"]]:
    #for gpsn in [["f", "rside", 1949, 7677, "31to40", "train"]]:

    #for gpsn in [["m", "lay", 1946, 8493, "1to10", "train"]]:
    #for gpsn in [["m", "lay", 2192, 8493, "11to20", "train"]]:
    #for gpsn in [["m", "lay", 2178, 8493, "21to30", "train"]]:
    #for gpsn in [["m", "lay", 2177, 8493, "31to40", "train"]]:

    #for gpsn in [["m", "lside", 1731, 7761, "1to10", "train"]]:
    #for gpsn in [["m", "lside", 2007, 7761, "11to20", "train"]]:
    ...
    #for gpsn in [["f", "lay", 2198, 6608, "41to50", "train"]]:
    #for gpsn in [["f", "lay", 2197, 6608, "51to60", "train"]]:
    #for gpsn in [["f", "lay", 2213, 6608, "61to70", "train"]]:

    #for gpsn in [["f", "lside", 2091, 6158, "41to50", "train"]]:
    #for gpsn in [["f", "lside", 2053, 6158, "51to60", "train"]]:
    #for gpsn in [["f", "lside", 2014, 6158, "61to70", "train"]]:

    #for gpsn in [["f", "rside", 1976, 6006, "41to50", "train"]]:
    #for gpsn in [["f", "rside", 2043, 6006, "51to60", "train"]]:
    #for gpsn in [["f", "rside", 1987, 6006, "61to70", "train"]]:

    #for gpsn in [["m", "lay", 2195, 6597, "41to50", "train"]]:
    #for gpsn in [["m", "lay", 2199, 6597, "51to60", "train"]]:
    #for gpsn in [["m", "lay", 2203, 6597, "61to70", "train"]]:

    #for gpsn in [["m", "lside", 2049, 5935, "41to50", "train"]]:
    #for gpsn in [["m", "lside", 1952, 5935, "51to60", "train"]]:
    #for gpsn in [["m", "lside", 1934, 5935, "61to70", "train"]]:

    #for gpsn in [["m", "rside", 1904, 5817, "41to50", "train"]]:
    #for gpsn in [["m", "rside", 1973, 5817, "51to60", "train"]]:
    #for gpsn in [["m", "rside", 1940, 5817, "61to70", "train"]]:

    for gpsn in [["f", "lay", 2184, 2184, "71to80", "train"]]:
    #for gpsn in [["f", "lside", 2058, 2058, "71to80", "train"]]:
    #for gpsn in [["f", "rside", 2010, 2010, "71to80", "train"]]:
    #for gpsn in [["m", "lay", 2188, 2188, "71to80", "train"]]:
    #for gpsn in [["m", "lside", 2002, 2002, "71to80", "train"]]:
    #for gpsn in [["m", "rside", 1939, 1939, "71to80", "train"]]:
    ...
    #for highct_setnum in [["445", "601", "1", "f", "_plo"],["1782", "2601", "2", "f", "_plo"],["1769", "2601", "4", "f", "_plo"],["560", "800", "1", "m", "_plo"],["1958", "2601", "2", "m", "_plo"]]:
        ...
        mesh_folder = FILEPATH+"data_BP/synth_meshes/"
        file_name = 'slp_' + subj_nums + '_' + posture + '_' + gender + '_' + str(num_resting_poses) +'_filtered'




        mesh_folder = mesh_folder + file_name
        mv = np.load(mesh_folder+"/"+file_name+"_mv.npy", allow_pickle = True)
        mf = np.load(mesh_folder+"/"+file_name+"_mf.npy", allow_pickle = True)
        hv = np.load(mesh_folder+"/"+file_name+"_hv.npy", allow_pickle = True)
        hf = np.load(mesh_folder+"/"+file_name+"_hf.npy", allow_pickle = True)
        bv = np.load(mesh_folder+"/"+file_name+"_bv.npy", allow_pickle = True)
        bf = np.load(mesh_folder+"/"+file_name+"_bf.npy", allow_pickle = True)
        k = np.load(mesh_folder+"/"+file_name+"_k.npy", allow_pickle = True)
        ...
        dat_f_synth = TensorPrepLib().load_files_to_database([[pmat_folder + dattype + '_slp_'  + posture + '_' + gender + '_71to80_' + str(num_databag_poses)+".p"]],\
        ...
        #sample_pimg_idx += 1904+1973#2049+1952#1904#1704+1927+1844#1731+2007+2002#1946+2192+2178#1805+2001+1922##1857+2087+2086#1944+2210+2201#1704+1927+1844# 1904+1973#2049+1952#1904+1973#2049+1952#2195+2199#1976 + 2043##
        for real_idx in range(0, np.shape(mv)[0]):



            for shape_param in range(10):
                m.betas[shape_param] = train_y_flat[real_idx][72+shape_param]
            for pose_param in range(72):
                m.pose[pose_param] = train_y_flat[real_idx][82+pose_param]


            human_verts_param = np.array(m.r)
            for s in range(0,3):
                human_verts_param[:, s] += (train_y_flat[real_idx][154+s])#this thing is off a bit.
            ...
            sample_idx = real_idx + 0


            #print sample_idx
            human_verts = np.array(hv[sample_idx, :, :])/2.58872 #np.load(mesh_folder + "human_mesh_verts_py.npy")/2.58872
            ...
            human_faces = np.array(hf[sample_idx, :, :]) #np.load(mesh_folder + "human_mesh_faces_py.npy")
            ...
            mattress_verts = np.array(mv[sample_idx, :, :])/2.58872 #np.load(mesh_folder + "mattress_verts_py.npy")/2.58872
            ...
            mattress_faces = np.array(mf[sample_idx, :, :]) #np.load(mesh_folder + "mattress_faces_py.npy")
            ...
            mat_blanket_verts = np.array(bv[sample_idx])/2.58872 #np.load(mesh_folder + "mat_blanket_verts_py.npy")/2.58872
            mat_blanket_verts = np.concatenate((mat_blanket_verts[:, 2:3],mat_blanket_verts[:, 0:1],mat_blanket_verts[:, 1:2]), axis = 1)

            mat_blanket_faces = np.array(bf[sample_idx]) #np.load(mesh_folder + "mat_blanket_faces_py.npy")
            ...
            for i in range(0, 6534):
                if mattress_verts[i, 0] > 4.8 and mattress_verts[i, 1] > .37 and mattress_verts[i, 2] > 2.2:
                    print(mattress_verts[i, :], i)
        
            for i in range(3000):
                idx = 1
                if mattress_faces[i, 0] == idx or mattress_faces[i, 1] == idx or mattress_faces[i, 2] == idx:
                    print('***', mattress_faces[i, :])
            ...
            if load_real == True:
                pimage_real_curr_uncover = pimage_real['uncover'][real_idx, :, :]
                pimage_real_curr_cover = pimage_real['cover2'][real_idx, :, :]

                depth_real_curr_uncover = depth_real['uncover'][real_idx, :, :]
                depth_real_curr_cover = depth_real['cover2'][real_idx, :, :]

                color_real_curr_uncover = color_real['uncover'][real_idx, :, :]
                color_real_curr_cover = color_real['cover2'][real_idx, :, :]
            else:
            ...
            if sample_idx%200 == 0:
                ...
                #                                     depth_im_list[-1], color_im_list[-1], pimage_real_curr_cover, depth_real_curr_cover, color_real_curr_cover, 2200, [True, sample_idx, file_name+"_slp"])
                VisualizationLib().plot_pimage_depth(pimage_synth_curr, depth_im_list_onlyhuman[-1], color_im_list_onlyhuman[-1], pimage_real_curr_uncover, depth_real_curr_uncover, color_real_curr_uncover, \
                                                     depth_im_list[-1], color_im_list[-1], pimage_real_curr_cover, depth_real_curr_cover, color_real_curr_cover, 2200, [False, sample_idx, file_name+"_slp"])
                #except:
                #    pass
                print('got here')
