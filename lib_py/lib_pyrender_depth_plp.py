...
class pyRenderMesh():
    def __init__(self, render):
        ...
        if True: #render == True:
            ...
            self.human_mat_lite = pyrender.MetallicRoughnessMaterial(baseColorFactor=[0.1, 0.1, 0.1 ,0.9])#[0.05, 0.05, 0.8, 0.0])#
        ...
        self.average_pooling = nn.AvgPool2d(5, stride = 1, padding=2)
    ...
    def get_transforms(self):
        ...
        transform_dict['smpl_seg_est'] = np.identity(4)
        transform_dict['smpl_seg_est'][1, 3] = 2.0  # move things over
        ...
        transform_dict['smpl_seg_gt'] = np.identity(4)
        transform_dict['smpl_seg_gt'][0, 3] = -2.0  # move things over
        transform_dict['smpl_seg_gt'][1, 3] = 2.0  # move things over
    ...
    def convert_smplarr_to_segmented_mesh(self, smpl_verts, smpl_faces, material, transform):
        smpl_trimesh_list = []
        smpl_mesh_list = []
        if smpl_verts is not None:
            smpl_verts_quad = np.concatenate((smpl_verts, np.ones((smpl_verts.shape[0], 1))), axis = 1)
            ...
            human_mesh_vtx_all, human_mesh_face_all = LPF.get_human_mesh_parts(smpl_verts, smpl_faces, segment_limbs=True, segment_type='pressure')
            for idx in range(len(human_mesh_vtx_all)):
                print(idx, np.shape(human_mesh_vtx_all[idx]), np.shape(human_mesh_face_all[idx]))

                tm_curr = trimesh.base.Trimesh(vertices=np.array(human_mesh_vtx_all[idx]), faces = np.array(human_mesh_face_all[idx]), process = False)
                smpl_trimesh_list.append(tm_curr)
                smpl_mesh_list.append(pyrender.Mesh.from_trimesh(tm_curr, material=material, wireframe=True))
    ...
    def get_smpl_pressure_proj(self, smpl_trimesh, pmat_est, transform_prev, transform, vertex_pressure = None, segment_limbs=False, segment_type='joints'):
        ...
        vertex_normals_pimg = np.array(smpl_trimesh.vertex_normals)
        vertices_pimg = np.array(smpl_trimesh.vertices)
        faces_pimg = np.array(smpl_trimesh.faces)
        ...
        norm_area_avg_all = LPF.get_triangle_area_vert_weight(vertices_pimg, faces_pimg, None)

        if vertex_pressure is None:
            vertex_pressure = []
                ...
                try:
                    if vertex_normals_pimg[all_vert_idx, 2] > 0:
                        ...
                        if cancel_pressure == 1:
                            vertex_pressure.append(0.0)
                        else:
                            #print(vertices_pimg[all_vert_idx])
                            if vertices_pimg[all_vert_idx][2] >= 0: #check if its above the height of the bed
                                vertex_pressure.append(pmat_est[color_idx_y, color_idx_x])
                            else:
                                vertex_pressure.append(0.0)
                    else:
                        vertex_pressure.append(0.0)
        ...
        vertex_pressure_norm = vertex_pressure*norm_area_avg_all

        verts_color_error = np.array(vertex_pressure) /23.

        all_verts = np.array(vertices_pimg)
        faces_red = np.array(faces_pimg)

        print(np.shape(verts_color_error), 'correct shape')
        verts_color_jet = np.clip(cm.jet(verts_color_error)[:, 0:3], a_min = 0.0, a_max = 1.0)


        smpl_trimesh_list_pimg = []
        smpl_mesh_list_pimg = []
        vertex_pressure_norm_list_ALL = []

        segment_metric_list = ['l_toes', 'r_toes', 'l_heel', 'r_heel', 'l_elbow', 'r_elbow', 'l_shoulder', 'r_shoulder',
                               'spine', 'head', 'l_hip', 'r_hip', 'sacrum', 'ischium']
        segment_metric_dict = {}


        for segment_limbs_ct in [0, 1]:

            if segment_limbs_ct == 0:
                segment_limbs_curr = segment_limbs
            else:
                segment_limbs_curr = False


            all_verts_list, faces_red_list = LPF.get_human_mesh_parts(all_verts, faces_red, segment_limbs=segment_limbs_curr, segment_type=segment_type)
            verts_color_jet_list, _ = LPF.get_human_mesh_parts(verts_color_jet, faces_red, segment_limbs=segment_limbs_curr, segment_type=segment_type)
            vertex_pressure_list, _ = LPF.get_human_mesh_parts(np.array(vertex_pressure), faces_red, segment_limbs=segment_limbs_curr, segment_type=segment_type)
            vertex_pressure_norm_list, _ = LPF.get_human_mesh_parts(vertex_pressure_norm, faces_red, segment_limbs=segment_limbs_curr, segment_type=segment_type)
            ...
            for idx in range(len(all_verts_list)):


                #print(np.shape(vertex_pressure_norm_list[idx]), 'norm shape')
                #print(verts_color_jet_list[idx].shape, segment_metric_list[idx], np.max(vertex_pressure_list[idx]), 'color list shape')
                if len(all_verts_list) > 1 and segment_limbs_curr == True:


                    segment_metric_dict[segment_metric_list[idx]] = np.max(vertex_pressure_list[idx])*133.32/1000. #do this for peak comparison



                    #norm_area_avg = LPF.get_triangle_area_vert_weight(all_verts_list[idx], faces_red_list[idx], None)
                    #vertex_pressure_norm_seg = vertex_pressure_list[idx]*norm_area_avg
                    #segment_metric_dict[segment_metric_list[idx]] = np.array(vertex_pressure_norm_seg)#vertex_pressure_norm_list[idx] #do this for v2vP comparison



                    #print(idx, np.shape(vertex_pressure_norm), np.shape(vertex_pressure_norm_list[idx]))


                    '''if len(old_t1) > 0 and old_t1 is not None:
                        print("now we can better decide what to viz")
                        print(segment_metric_list[idx], segment_metric_dict[segment_metric_list[idx]], old_t4[idx], threshold)
                        if old_t4[idx] == 0: continue
                        if segment_metric_dict[segment_metric_list[idx]] < threshold: continue

                    else:
                        print("first round")


                    if segment_metric_dict[segment_metric_list[idx]] >= 1:
                        self.t1.append(1)
                        #color = np.clip(cm.jet(np.array([1.*(1000/133.32) / 23.]))[:, 0:3], a_min=0.0, a_max=1.0)
                    else:
                        self.t1.append(0)
                        #color = np.clip(cm.jet(np.array([0.*(1000/133.32) / 23.]))[:, 0:3], a_min=0.0, a_max=1.0)
                    if segment_metric_dict[segment_metric_list[idx]] >= 2:
                        self.t2.append(1)
                    else:
                        self.t2.append(0)
                    if segment_metric_dict[segment_metric_list[idx]] >= 3:
                        self.t3.append(1)
                    else:
                        self.t3.append(0)

                    if segment_metric_dict[segment_metric_list[idx]] >= 4:
                        self.t4.append(1)
                    else:
                        self.t4.append(0)'''

                    #print('color:', color)
                    #for i in range(verts_color_jet_list[idx].shape[0]):
                    #    verts_color_jet_list[idx][i] = color[0]


                vertex_pressure_norm_list_ALL.append(vertex_pressure_norm_list[idx])

                verts_color_jet_top = np.concatenate((verts_color_jet_list[idx], np.ones((verts_color_jet_list[idx].shape[0], 1))*0.9), axis = 1)
                verts_color_jet_bot = np.concatenate((verts_color_jet_list[idx]*0.3, np.ones((verts_color_jet_list[idx].shape[0], 1))*0.9), axis = 1)

                faces_underside = np.concatenate((faces_red_list[idx][:, 0:1],
                                                  faces_red_list[idx][:, 2:3],
                                                  faces_red_list[idx][:, 1:2]), axis = 1) + all_verts_list[idx].shape[0]

                human_vtx_both_sides = np.concatenate((all_verts_list[idx], all_verts_list[idx]+0.0001), axis = 0)
                human_mesh_faces_both_sides = np.concatenate((faces_red_list[idx], faces_underside), axis = 0)
                ...
                smpl_trimesh_list_pimg.append(tm_curr)

                if segment_limbs_ct == 0:
                    smpl_mesh_list_pimg.append(pyrender.Mesh.from_trimesh(smpl_trimesh_list_pimg[-1], smooth=False))
                else:
                    if segment_limbs == True:
                        smpl_mesh_list_pimg.append(pyrender.Mesh.from_trimesh(smpl_trimesh_list_pimg[-1], material=self.human_mat_lite, wireframe=True))
                    else:
                        smpl_mesh_list_pimg.append(pyrender.Mesh.from_trimesh(smpl_trimesh_list_pimg[-1], material=self.human_mat, wireframe=True))
            ...
            #for idx in range(len(all_verts_list)):
            #    print(segment_metric_list[idx])


        return smpl_trimesh_list_pimg, smpl_mesh_list_pimg, vertex_pressure, vertex_pressure_norm_list_ALL, segment_metric_dict




    def get_2D_pmat_peaks(self, pmat):
        pmat_alter_array = np.zeros((3, np.shape(pmat)[0]+2,np.shape(pmat)[1]+2))

        pmat_alter_array[0, 1:-1, 1:-1] = np.array(pmat)
        for i in range(np.shape(pmat)[0]):
            for j in range(np.shape(pmat)[1]):
                pmat_alter_array[1, 1+i, 1+j] = np.max([pmat_alter_array[0, 0+i, 0+j],
                                                        pmat_alter_array[0, 0+i, 1+j],
                                                        pmat_alter_array[0, 0+i, 2+j],
                                                        pmat_alter_array[0, 1+i, 0+j],
                                                        pmat_alter_array[0, 1+i, 2+j],
                                                        pmat_alter_array[0, 2+i, 0+j],
                                                        pmat_alter_array[0, 2+i, 1+j],
                                                        pmat_alter_array[0, 2+i, 2+j]])

        pmat_alter_array[2, :, :] = pmat_alter_array[0, :, :] - pmat_alter_array[1, :, :]
        pmat_alter_array[2, :, :][pmat_alter_array[2, :, :] >= 0] = 1
        pmat_alter_array[2, :, :][pmat_alter_array[2, :, :] < 0] = 0

        pmat_peak = pmat_alter_array[2, 1:-1, 1:-1]*pmat_alter_array[0, 1:-1, 1:-1]
        #pmat_peak[pmat_peak < 30] = 0
        #print(pmat_peak, np.max(pmat_peak))
    ...
    def render_mesh_pc_bed_pyrender_everything(self, smpl_verts, smpl_faces, camera_point, RESULTS_DICT,
        ...
        #print(pmat, "pmat", np.max(pmat))
        #print(pmat_est, "pmat est", np.max(pmat))


        #pmat_peak = self.get_2D_pmat_peaks(pmat)
        #pmat_est_peak = self.get_2D_pmat_peaks(pmat_est)
        #pmat_est_peak = self.get_2D_pmat_peaks(pmat)
        #pmat_est = self.get_2D_pmat_peaks(pmat)
        '''pmat[pmat < 1] = 0
        pmat_est[pmat_est < 1] = 0

        pmat_interim = torch.Tensor(np.array(pmat)).unsqueeze(0).unsqueeze(0)
        pmat_conv = self.average_pooling(pmat_interim).squeeze().numpy()#*0.0
        pmat_interim_bin = torch.Tensor(np.array(pmat)).unsqueeze(0).unsqueeze(0)
        pmat_interim_bin[pmat_interim_bin != 0] = 1.
        pmat_conv_bin = self.average_pooling(pmat_interim_bin).squeeze().numpy()
        #print(pmat_conv)
        #print(pmat_conv_bin)
        pmat_conv = pmat_interim_bin.squeeze().numpy()*pmat_conv/(pmat_conv_bin+0.00001)




        pmat_interim_est = torch.Tensor(np.array(pmat_est)).unsqueeze(0).unsqueeze(0)
        pmat_est_conv = self.average_pooling(pmat_interim_est).squeeze().numpy()#*0.0
        pmat_est_interim_bin = torch.Tensor(np.array(pmat_est)).unsqueeze(0).unsqueeze(0)
        pmat_est_interim_bin[pmat_est_interim_bin != 0] = 1.
        pmat_est_conv_bin = self.average_pooling(pmat_est_interim_bin).squeeze().numpy()
        #print(pmat_est_conv)
        #print(pmat_est_conv_bin)
        pmat_est_conv = pmat_est_interim_bin.squeeze().numpy()*pmat_est_conv/(pmat_est_conv_bin+0.00001)'''



        #print(pmat_conv.shape, 'conv shape')
        ...
        if project_pressure == False:
            smpl_trimesh_list_est = []
            smpl_trimesh_list_GT = []


        #get the projections
        _, smpl_mesh_list_pimg_est, vertex_pressure_est, vertex_pressure_norm_est, _ = self.get_smpl_pressure_proj(smpl_trimesh_list_est[0], pmat_est,
                                                                                        transform_prev=transform_dict['smpl_pmat_est'],
                                                                                        transform=transform_dict['pressure_proj_est'])

        _, smpl_mesh_list_pimg_GT, vertex_pressure_GT, vertex_pressure_norm_GT, _ = self.get_smpl_pressure_proj(smpl_trimesh_list_GT[0], pmat,
                                                                                        transform_prev=transform_dict['smpl_pmat_gt'],
                                                                                        transform=transform_dict['pressure_proj_gt'])

        _, smpl_mesh_seg_list_pimg_GT, _, _, segment_metric_dict_GT = self.get_smpl_pressure_proj(smpl_trimesh_list_GT[0], pmat,
                                                                        transform_prev=transform_dict['smpl_pmat_gt'],
                                                                        transform=transform_dict['smpl_seg_gt'],
                                                                        vertex_pressure=vertex_pressure_GT,
                                                                        segment_limbs=True,
                                                                        segment_type='pressure')


        _, smpl_mesh_seg_list_pimg_est, _, _, segment_metric_dict_est = self.get_smpl_pressure_proj(smpl_trimesh_list_est[0], pmat_est,
                                                                        transform_prev=transform_dict['smpl_pmat_est'],
                                                                        transform=transform_dict['smpl_seg_est'],
                                                                        vertex_pressure=vertex_pressure_est,
                                                                        segment_limbs=True,
                                                                        segment_type='pressure')

        #for pressure_thresh in [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]:


        #for item in segment_metric_dict_est:
        #    try:
        #        RESULTS_DICT[item+'_vertex_pressure_list_EST'].append(segment_metric_dict_est[item])
        #    except:
        #        RESULTS_DICT[item + '_vertex_pressure_list_EST'] = []
        #        RESULTS_DICT[item+'_vertex_pressure_list_EST'].append(segment_metric_dict_est[item])


        for item in segment_metric_dict_est:
            try:
                RESULTS_DICT[item+'_maxp_EST'].append(segment_metric_dict_est[item])
            except:
                RESULTS_DICT[item + '_maxp_EST'] = []
                RESULTS_DICT[item+'_maxp_EST'].append(segment_metric_dict_est[item])


            #print(item, segment_metric_dict_est[item], segment_metric_dict_GT[item])
            #total_ct += 1.
            #if segment_metric_dict_est[item] >= pressure_thresh and segment_metric_dict_GT[item] >= pressure_thresh:
            #    success_ct += 1.


        #for item in segment_metric_dict_GT:
        #    try:
        #        RESULTS_DICT[item+'_vertex_pressure_list_GT'].append(segment_metric_dict_GT[item])
        #    except:
        #        RESULTS_DICT[item + '_vertex_pressure_list_GT'] = []
        #        RESULTS_DICT[item+'_vertex_pressure_list_GT'].append(segment_metric_dict_GT[item])


        for item in segment_metric_dict_GT:
            try:
                RESULTS_DICT[item+'_maxp_GT'].append(segment_metric_dict_GT[item])
            except:
                RESULTS_DICT[item + '_maxp_GT'] = []
                RESULTS_DICT[item+'_maxp_GT'].append(segment_metric_dict_GT[item])
        ...
        #_, smpl_mesh_list_pimg_est, _ = self.get_smpl_pressure_proj(smpl_trimesh_list_est, pmat_est,
        #                                                                transform_prev=transform_dict['smpl_pmat_est'],
        #                                                                transform=transform_dict['smpl_seg_est'])


        if len(smpl_trimesh_list_est) > 0:
            RESULTS_DICT['vertex_pressure_list_EST'].append(vertex_pressure_norm_est[0])
            RESULTS_DICT['vertex_pressure_list_GT'].append(vertex_pressure_norm_GT[0])
        ...
        if self.render == True:
            ...
            if self.first_pass == True:
                ...
                for mesh_part_seg in smpl_mesh_seg_list_pimg_est:
                    self.scene.add(mesh_part_seg)

                for mesh_part_seg_GT in smpl_mesh_seg_list_pimg_GT:
                    self.scene.add(mesh_part_seg_GT)

                for mesh_part_pimg in smpl_mesh_list_pimg_est:
                ...
                self.node_list_seg_GT = []
                for mesh_part_seg_GT in smpl_mesh_seg_list_pimg_GT:
                    for node in self.scene.get_nodes(obj=mesh_part_seg_GT):
                        self.node_list_seg_GT.append(node)
                ...
                for idx in range(len(smpl_mesh_list_pimg_est)):
                    self.scene.remove_node(self.node_list_pimg[idx])
                    self.scene.add(smpl_mesh_list_pimg_est[idx])
                    for node in self.scene.get_nodes(obj=smpl_mesh_list_pimg_est[idx]):
                ...
                #reset the segmented projection
                for idx in range(len(smpl_mesh_seg_list_pimg_est)):
                    self.scene.remove_node(self.node_list_seg[idx])
                    self.scene.add(smpl_mesh_seg_list_pimg_est[idx])
                    for node in self.scene.get_nodes(obj=smpl_mesh_seg_list_pimg_est[idx]):
                        self.node_list_seg[idx] = node

                #reset the segmented projection ground truth
                for idx in range(len(smpl_mesh_seg_list_pimg_GT)):
                    self.scene.remove_node(self.node_list_seg_GT[idx])
                    self.scene.add(smpl_mesh_seg_list_pimg_GT[idx])
                    for node in self.scene.get_nodes(obj=smpl_mesh_seg_list_pimg_GT[idx]):
                        self.node_list_seg_GT[idx] = node
