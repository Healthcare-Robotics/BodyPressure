...
class MeshDepthLib():
    ...
    def __init__(self, loss_vector_type, batch_size, use_cuda, verts_list, CTRL_PNL = None):
        ...
        if use_cuda == True:
            ...
            print('######################### CUDA is available! ############################# SLP: ', slp)
        else:
            self.GPU = False
            # Use for CPU
            dtype = torch.FloatTensor
            dtypeInt = torch.LongTensor
            print('############################## USING CPU ################################# SLP: ', slp)
        self.dtype = dtype
        ...
        if self.loss_vector_type == 'anglesDC':
            if slp == 'real' or slp == 'synth':
                print(" USING SLP BOUNDS")
                self.bounds = torch.Tensor(
                    np.array([[-0.5933865286111969, 0.5933865286111969], [-2*np.pi, 2*np.pi], [-1.215762200416361, 1.215762200416361],
                              [-1.87, 0.4], [-0.31, 0.44],[-0.53, 0.94],
                              [-1.87, 0.4], [-0.44, 0.31],[-0.94, 0.53],
                              [-np.pi / 3, np.pi / 3], [-np.pi / 36, np.pi / 36], [-np.pi / 36, np.pi / 36],
                              [-0.01, 2.57], [-0.01, 0.01], [-0.01, 0.01],  # knee
                              [-0.01, 2.57], [-0.01, 0.01], [-0.01, 0.01],
                              [-np.pi / 3, np.pi / 3], [-np.pi / 36, np.pi / 36], [-np.pi / 36, np.pi / 36],
                              ...
                              [-0.45, 0.21], [-1.11, 0.69], [-0.63, 0.74],
                              [-0.45, 0.21], [-0.69, 1.11], [-0.74, 0.63],
                              [-np.pi / 3, np.pi / 3], [-np.pi / 36, np.pi / 36], [-np.pi / 36, np.pi / 36],  # head
                              [-0.82, 0.73], [-1.35, 0.50], [-1.30, 0.75],
                              [-0.82, 0.73], [-0.50, 1.35], [-0.75, 1.30],
                              [-0.01, 0.01], [-2.67, 0.1], [-0.01, 0.01],  # elbow
                              [-0.01, 0.01], [-0.1, 2.67], [-0.01, 0.01],  # elbow
                              [-np.pi / 6, np.pi / 6], [-np.pi / 6, np.pi / 6], [-np.pi / 6, np.pi / 6],
        ...
        if verts_list == "all":
            ...
            if loss_vector_type == 'anglesR' or loss_vector_type == 'anglesDC' or loss_vector_type == 'anglesEU':
                ...
                self.R_used = 10
                self.shapedirs = self.shapedirs_repeat.permute(1, 0, 2, 3, 4).view(self.N, 2, self.B * self.R * self.D)
                ...
                self.v_template = torch.cat((self.v_template_repeat_f, self.v_template_repeat_m),
                ...
                self.J_regressor = torch.cat((self.J_regressor_repeat_f, self.J_regressor_repeat_m),
                                                    0)  # this is 2 x N x R x 24
                self.J_regressor = self.J_regressor.permute(1, 0, 2, 3).view(self.N, 2, self.R * 24)
                ...
                self.posedirs = torch.cat((self.posedirs_repeat_f, self.posedirs_repeat_m), 0)
    ...
    def PMR(self, verts, bed_angle_batch, bed_vertical_shift_est, CTRL_PNL, get_mesh_bottom_dist = True, is_training = False):
        ...
        if get_mesh_bottom_dist == False:
            ...
            #if SLP is synth then we need to shift xy offset back to where it was. otherwise if its real keep it the same. we only use this when computing forward pass in beteween mods 1 and 2 anyway
            #if its during training there is currently nothing implemented to be able to switch between


            if CTRL_PNL['slp'] == 'mixedreal' and is_training == True:
                print("We have not yet implemented way to offset the reconstructed depth maps when there is a mixed bag of real and synthetic during training. Exiting.")
                sys.exit()
            elif CTRL_PNL['slp'] == 'synth':
                verts_rot_taxel[:, :, 0] += float(CTRL_PNL['x_y_offset_synth'][0]/1000.)
                verts_rot_taxel[:, :, 1] -= float(CTRL_PNL['x_y_offset_synth'][1]/1000.)
                verts_taxel[:, :, 0] += float(CTRL_PNL['x_y_offset_synth'][0]/1000.)
                verts_taxel[:, :, 1] -= float(CTRL_PNL['x_y_offset_synth'][1]/1000.)
            else:
                pass


            #verts_taxel[:, :, 2] += bed_vertical_shift_est/1000. #this is for depth map reconstruction. it is GLOBAL. leave bed vertical shift commented out.
            verts_taxel[:, :, 2] += 0.4064 #add an arbitrary fudge factor (16 inches in this case... ) so that we can get joints below the surface of the bed and not cutoff too early.
            #we could alternatively flip the sign and make it a distance from the overhead camera rather than a distance from the ground (its between the two). but then the edges of the
            #body would not be very smooth, because we are modeling the distance to bed surface as "zero" or a void.
            pass
        else:
            #if SLP is synth OR real then we need to shift back from the synthetic offset.
            verts_rot_taxel[:, :, 0] += float(CTRL_PNL['x_y_offset_synth'][0]/1000.)
            verts_rot_taxel[:, :, 1] -= float(CTRL_PNL['x_y_offset_synth'][1]/1000.)
            verts_taxel[:, :, 0] += float(CTRL_PNL['x_y_offset_synth'][0]/1000.)
            verts_taxel[:, :, 1] -= float(CTRL_PNL['x_y_offset_synth'][1]/1000.)
            verts_taxel[:, :, 2] -= bed_vertical_shift_est/1000. #this is for pressure map reconstruction. it is relative to the bed height, so we need to shift the verts accordingly.
